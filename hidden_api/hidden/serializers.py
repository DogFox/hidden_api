...
class MembershipSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Membership
        fields = ('id', 'secretbox', 'santa', 'member', 'exception', 'santa_name',
                  'member_name', 'member_email', 'member_wishes', 'santa_wishes')


class SecretBoxSerializer(serializers.ModelSerializer):
    memberships = MembershipSerializer(
        source='secretboxs', many=True, read_only=True)
    ...
    def get_santas(self, obj):
        ...
        memberships = Membership.objects.filter(
            santa__in=members, secretbox=obj)
        serializer = MembershipSerializer(memberships, many=True)
        return serializer.data

    class Meta:
        model = SecretBox
        fields = ('id', 'name', 'admin', 'description',
                  'memberships', 'santas', 'limit', 'limitValue')
